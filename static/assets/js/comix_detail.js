...
document.getElementById("commentForm").addEventListener("submit", function(event) {
    ...
    // Добавляем CSRF-токен в данные формы
    const csrfToken = document.querySelector('[name=csrfmiddlewaretoken]').value;
    formData.append('csrfmiddlewaretoken', csrfToken);

    // Отправляем запрос с использованием AJAX
        $.ajax({
            url: window.location.href, // Укажите адрес сервера
            type: 'POST',
            data: formData,
...
function updateComments() {
    $.ajax({
        url: window.location.href, // Отправляем запрос на текущий URL-адрес страницы
        type: 'GET', // Используем метод GET для получения данных
        dataType: 'json', // Ожидаем, что данные будут в формате JSON
        success: function(data) {
            ...
            newComments.forEach(comment => {
                ...
                div.innerHTML = `
                    <div>
                        <h5 id="scroll${comment.id}">${comment.user.username}</h5>
                        <p>${comment.text}</p>
                    </div>
                `;
